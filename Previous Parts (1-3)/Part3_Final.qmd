...
head(paragraph_list) 
#there's an extra space in between each paragraph that needs to be cleaned out
```
...
#removes extra rows that were only recorded on some pages


# adding in missing rows -------------------------------------------------------
indexes <- c(9, 11, 20, 26, 37, 40, 107, 109, 110, 112, 113, 114, 115) #
#indexes of parks with missing paragraphs: cascade river, father hennepin, 
#helmer myre, kadunce river, o l kipp, st croix wild river, boekholt grove, 
#cooks, coon lake, little elk, malardi lake, talcot lake, teiken-dalve

for (i in 1:13) {
  park_par_tbl <- append(park_par_tbl, "", after = indexes[i])
} #adds in missing rows based on indexes of parks with missing paragraphs.
  #These will become NAs later.
